...
namespace GachaApplication
{
    class Program
    {
        ...
        {
            // Split the item string by space
            string[] parts = item.Split(' ');

            // Print out the parts of the item string (for debugging)
            foreach (string part in parts)
            {
                Console.WriteLine(part);
            }

            // Extract the last part which should be the item number
            string lastPart = parts[parts.Length - 1];

            // Attempt to parse the last part as an integer
            int itemNumber;
            if (!int.TryParse(lastPart, out itemNumber))
            {
